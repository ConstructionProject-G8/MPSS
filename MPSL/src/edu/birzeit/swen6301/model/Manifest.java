...
/**
 * To encapsulate and organize all related specifications (segments & links) of certain manifest file contents.<p>
 * <p>
 * Main Functions:<p>
 * 1. Call mirror's stream to download manifest contents by using from URL.<p>
 * 2. Split segments as text array from whole manifest text that has been fetched from that URL.<p>
 * 3. Bind segment objects into attached list.<p>
 * <p>
 * @author
 */
public class Manifest implements IPart {
	...
	public String load() throws StreamException, ParseException {
		...
		if (!segment.getOpenedMirror().hasManifest()) {		// If direct link (not manifest).
			LogHandler.writeEvent("Manifest event: This ("+ this.getUrl()+ ") isn't manifest, it is direct link.");
			return this.getUrl();
		}
			
		contents = new String(segment.download());	// Download manifest contents.
	...
	/**
	 * Open related stream for first segment.
	 * 
	...
	/**
	 * Used to close input stream and disconnect (http or url) connection of this all related segments.
	 */
	@Override
	public void closeStream() {
		if (getSegments() == null) return;	// If Empty.
		
		// Segments loop.
		for (int i=0; i < getSegments().size(); i++) getSegments().get(i).closeStream();
	}
