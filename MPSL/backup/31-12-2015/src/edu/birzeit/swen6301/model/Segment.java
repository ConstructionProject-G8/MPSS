...
public class Segment implements IPart, Comparator, Comparable {
	 ...
	 * @throws StreamException
	 */
	public byte[] download(boolean doRecursive) throws StreamException {
		if (getMirrors() == null) return new byte[0];
		...
		for (int i=0; i < getMirrors().size(); i++) {
			try {
				...
				this.setBytes(mirror.download(doRecursive));		// Download mirror.
				...
				// Write error in log.
				LogHandler.writeError("Mirror stream error: ("+ mirror.getUrl()+ ") download failure, "+ e.getMessage());
			} catch (ParseException e) {
				// Write error in log.
				LogHandler.writeError("Mirror parse error: ("+ mirror.getUrl()+ ") download failure, "+ e.getMessage());
			}		
		}
