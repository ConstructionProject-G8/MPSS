...
public class Manifest implements IPart {
	...
	private static Map<String, IPart> manifests;
	...
	/**
	 * Used to put part as IPart type in global set with unique key (URL String).
	 * 
	 * @param url
	 * @param part
	 */
	public static void put(String url, IPart part) {
		if (manifests == null) manifests = new HashMap<String, IPart>();
		
		if (url != null && find(url) == null) manifests.put(url, part);
	}

	/**
	 * Used to find part in global set by unique key (URL String).
	 * 
	 * @param url
	 * @return
	 */
	public static IPart find(String url) {
